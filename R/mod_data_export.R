...
mod_data_export_ui <- function(id) {
  ...
  tabItem(
    tabName = "data_export",
    fluidRow(
      # No column() wrapper: the box already renders as col-sm-12; nesting it
      # double-applies the gutter and misaligns this tab's content edge with
      # every other tab.
      theme_box(
        title = "Data & Export",
        icon = icon("download"),
                  ...
                  tabPanel(
                    ...
                    div(
                      ...
                      div(
                        ...
                        browser_download_button(ns("download_cell_metrics"), "Download CSV", class = "btn-primary btn-sm")
                      ),
                  ...
                  tabPanel(
                    ...
                    div(
                      ...
                      div(
                        ...
                        browser_download_button(ns("download_summary"), "Download CSV", class = "btn-primary btn-sm")
                      ),
                  ...
                  tabPanel(
                    ...
                    div(
                      ...
                      div(
                        ...
                        browser_download_button(ns("download_timecourse"), "Download CSV", class = "btn-primary btn-sm")
                      ),
                  ...
                  tabPanel(
                    ...
                    div(
                      ...
                      div(
                        ...
                        browser_download_button(ns("download_raw"), "Download CSV", class = "btn-primary btn-sm"),
                        browser_download_button(ns("download_raw_all"), "Download All (ZIP)", class = "btn-default btn-sm")
                      ),
...
mod_data_export_server <- function(id, rv, metrics_plot_reactive, heatmap_plot_reactive, time_course_plot_reactive) {
  moduleServer(id, function(input, output, session) {
    ...
    output$timecourse_summary_table <- DT::renderDT(server = FALSE, {
      ...
      DT::datatable(
        ...
        # Format the numeric column to display strings: the CSV/Excel button
        # export crashes on raw numeric cells, and every other table already
        # goes through formatRound
        DT::formatRound(columns = "Time", digits = 2)
    })

    output$raw_data_table <- DT::renderDT(server = FALSE, {
    ...
    register_browser_download(input, session, "download_cell_metrics",
      filename = function() {
    ...
    register_browser_download(input, session, "download_summary",
      filename = function() {
    ...
    register_browser_download(input, session, "download_timecourse",
      filename = function() {
    ...
    register_browser_download(input, session, "download_raw",
      filename = function() {
        # Lead with the selected group's original file name
        build_export_filename(rv, parts = "processed", ext = "csv",
                              base = input$processed_data_group %||% "dataset")
      },
      content = function(file) {
    ...
    # Every group's processed wide CSV in one ZIP, so multi-file uploads
    # don't require cycling the dropdown
    all_processed_filename <- function() {
      # Base name already identifies the upload (file name or group count)
      build_export_filename(rv, parts = "processed_all", ext = "zip")
    }
    write_all_processed_zip <- function(file) {
      req(rv$dts, length(rv$dts) > 0)
      out_dir <- file.path(tempdir(), paste0("processed_all_", format(Sys.time(), "%H%M%S")))
      dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
      paths <- vapply(names(rv$dts), function(g) {
        safe <- gsub("[^A-Za-z0-9._-]+", "_", g)
        f <- file.path(out_dir, paste0(safe, "_processed.csv"))
    ...
    register_browser_download(input, session, "download_raw_all",
      filename = all_processed_filename, content = write_all_processed_zip
    )
    register_browser_download(input, session, "dl_processed_all_exp",
      filename = all_processed_filename, content = write_all_processed_zip
    )
      ...
      if (is.null(rv$dts) || length(rv$dts) == 0) {
        ...
        fluidRow(
          ...
          column(4, style = "border-left: 1px solid var(--color-gray-100); padding-left: 20px;",
            ...
            div(style = "display: flex; flex-direction: column; gap: 10px;",
              browser_download_button(ns("dl_timecourse_plot"), "Time Course Plot", class = "btn-primary btn-block"),
              browser_download_button(ns("dl_heatmap_plot"), "Heatmap Plot", class = "btn-primary btn-block"),
              browser_download_button(ns("dl_metrics_plot"), "Current Metrics Plot", class = "btn-primary btn-block"),
              tags$hr(style = "margin: 6px 0;"),
              browser_download_button(ns("dl_all_figures"), "Download All Figures (ZIP)", class = "btn-default btn-block")
            ),
    ...
    register_browser_download(input, session, "dl_processed_wide_exp",
      filename = function() {
        # Lead with the selected group's original file name
        build_export_filename(rv, parts = "processed", ext = "csv",
                              base = input$exp_dl_group %||% "data")
      },
      content = function(file) {
    ...
    register_browser_download(input, session, "dl_metrics_csv",
      filename = function() {
        n_cells <- if (!is.null(rv$metrics)) nrow(rv$metrics) else 0
    ...
    register_browser_download(input, session, "dl_summary_csv",
      filename = function() {
        build_export_filename(rv, parts = "timecourse_summary", ext = "csv")
      },
      content = function(file) {
    ...
    register_browser_download(input, session, "dl_timecourse_plot",
      filename = function() {
    ...
    register_browser_download(input, session, "dl_heatmap_plot",
      filename = function() {
    ...
    register_browser_download(input, session, "dl_metrics_plot",
      filename = function() {
    ...
    register_browser_download(input, session, "dl_all_figures",
      filename = function() {
      ...
      content = function(file) {
        ...
        if (length(files_to_zip) > 0) {
          write_zip_archive(file, files_to_zip)
        }
      }
    )
