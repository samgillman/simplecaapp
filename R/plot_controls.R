...
plot_display_accordion <- function(ns, prefix = "tc", expanded = TRUE, include_line_color = TRUE) {
    content_list <- list(
        checkboxInput(ns(paste0(prefix, "_show_traces")), "Show individual traces",
            value = FALSE
        ),
        ...
        checkboxInput(ns(paste0(prefix, "_show_ribbon")), "Show SEM ribbon",
            value = TRUE
        ),
        checkboxInput(ns(paste0(prefix, "_show_source_means")), "Show per-file averages",
            value = TRUE
        ),
...
plot_export_accordion <- function(ns, prefix = "tc", expanded = FALSE,
    ...
    accordion(
        ...
        content = div(
            ...
            browser_download_button(ns(download_id), download_label, class = "btn-primary", style = "width: 100%; margin-top: 8px;")
        )
    )
}
