...
- **Features:**
  ...
  - **Helper text:** "Wide format: optional Time or Frame column, plus one column per cell trace."
- **JavaScript Enhancement:** Custom script that makes entire file input area clickable (overlay technique with opacity: 0)

#### 2.1.2 Baseline Frame Window
- **Estimator:** For each cell, F₀ is the mean fluorescence across the selected baseline frames.
- **Control:** `sliderInput` for "Baseline Window (frames)"
- **Initial Range:** 1 to 100 frames; the upper bound adapts to the shortest uploaded file
- **Default Value:** `c(1, 20)` (start frame, end frame)
- **Step:** 1 frame
- **Width:** 100% of column
- **Typed Bounds:** Synchronized `numericInput` controls for Start frame and End frame; changing either the slider or typed bounds updates the other controls
- **Metric Use:** The same window defines baseline noise and the initial frames excluded from response searches.
...
- **Content:**
  - **Sampling Rate Override:** `numericInput`
    ...
    - **Minimum:** 0.0001
    - **Step:** 0.1
    - **Helper Text:** "Used to convert a Frame column to seconds or generate Time when no valid Time column exists."
  - **Per-file Column Mapping:**
    - Time handling choices: Auto-detect, selected elapsed-time column, selected frame-index column, or generated Time
    - Displays the automatic Time/Frame detection result and numeric trace-candidate count
    - Multi-select exclusion control removes unwanted numeric columns before baseline correction and metrics
    - The selected Time/Frame source is never analyzed as a trace
...
3. **Timepoints Card**
   - **Label:** "Timepoints"
   - **Value:** Displays the row count for one file, a shared `timepoints/file` count when files match, or the per-file range when lengths differ
   - **Styling:** Same as Files card
...
- **Time detection:** Finds a Time-labelled column anywhere in the table and validates that it is numeric and monotonically increasing.
- **Frame conversion:** Finds a Frame-labelled column and converts it to elapsed seconds using the sampling rate.
- **ImageJ Index Inference:** An unnamed sequential integer first column (for example, `V1` after CSV name repair) is treated as Frame and converted using the sampling rate.
- **Auto-Generation:** If no Time, Frame, or inferred ImageJ index exists, or detected values are invalid, generates a time vector using sampling rate:
  - Formula: `seq(0, by = 1/sampling_rate, length.out = nrow)`
- **Trace preservation:** When Time is generated, no uploaded column is renamed or discarded; the generated Time column is prepended.
- **User confirmation:** Per-file mappings can override automatic detection with an explicit elapsed-time column, frame-index column, or generated Time. Explicit columns must be finite, numeric, and strictly increasing.
- **Trace exclusion:** User-excluded columns and the selected Time/Frame source are removed before numeric coercion, baseline estimation, ΔF/F₀ normalization, and metric calculation. Processing is rejected if no numeric trace remains.

#### 2.3.3 Baseline Calculation (F₀)

The application supports one baseline estimator: the mean of the selected frame range for each cell.
...
- **If F₀ ≤ 1e-6 or is not finite:** The cell is excluded and reported in an on-screen warning because ΔF/F₀ is undefined.
...
- **Metrics Displayed:** Peak ΔF/F₀, AUC, exact FWHM, censored FWHM lower bound, exact Half-Width, Ca²⁺ Entry Rate, Time to Peak, Time to 25/50/75% Peak, Rise Time, SNR
- **Censoring Display:** Exact FWHM/Half-Width rows identify recovered and right-censored counts. When no exact width exists, the cell reads `Not estimable — 0/N exact; N/N right-censored` instead of a bare `n=0`; the observed lower-bound row is labeled as censored.
...
#### 4.1.3 Shape Metrics
- **FWHM:** Exact Full Width at Half Maximum when both half-maximum crossings are observed
- **FWHM_Censored:** `TRUE` when the recording ends before the falling half-maximum crossing; `FALSE` for an exact FWHM; `NA` when width is otherwise undefined
- **FWHM_Lower_Bound:** Observed duration from the rising crossing to the final observed sample for a right-censored response
- **Half_Width:** Half of an exact FWHM (HWHM); missing for right-censored responses
...
- **Frame Semantics:** Baseline and response windows use original uploaded row numbers. Missing samples are ignored in place and never compact the trace.
...
- **Exact Result:** `FWHM = t_right - t_left` and `Half_Width = FWHM / 2`
- **Right-Censored Result:** If every observed sample from the peak through the end remains above half-max, `FWHM` and `Half_Width` are `NA`, `FWHM_Censored = TRUE`, and `FWHM_Lower_Bound = t_last - t_left`
- **Missing Samples:** Crossings are interpolated only between consecutive finite original frames; gaps are never bridged

#### 4.2.4 AUC Calculation
- **Method:** Trapezoidal rule
- **Formula:** `Σ[(y_i + y_{i+1})/2 × (t_{i+1} - t_i)]`
- **Integration:** Over consecutive finite original-frame pairs across the entire trace; intervals containing a missing sample do not contribute and are not bridged
...
- Exact responses: vertical dotted lines at both crossings, a two-sided FWHM arrow, and a Half-Width arrow
- Right-censored responses: a left crossing line and one-sided arrow to the final observed sample
- Exact labels: "FWHM = X.XX s", "Half-Width = X.XX s"
- Censored label: "Observed FWHM ≥ X.XX s" with a note that the right crossing was not observed
...
- **Critical:** Keeps metric explanations aligned with the frame window used during processing
...
- **Columns:** All metric columns plus Group, Cell, Cell_ID, Cell_Label
  - Censoring fields (`FWHM_Censored`, `FWHM_Lower_Bound`) are included so sustained responses remain distinguishable from undefined widths
- **Features:**
  - **Page Length:** 25 rows
...
- **Content:** Numbered list (6 steps):
  ...
  2. Process: Select the baseline frame window, click Process
...
- [x] Per-file Time/Frame confirmation and column exclusion
- [x] Three baseline correction methods
- [x] Synchronized slider and typed baseline frame bounds
